...
async function shuffleAndDrawCards(numCardsToDraw) {
  ...
  try {
    ...
    const deck = await $.getJSON(`${BASE_URL}/new/shuffle/?deck_count=1`); // shuffle cards
    const cardsData = await $.getJSON(`${BASE_URL}/${deck.deck_id}/draw/?count=${numCardsToDraw}`); // draw card
...
async function drawCard() {
  try {
    const cardsData = await $.getJSON(`${BASE_URL}/${deckID}/draw/?count=1`);
